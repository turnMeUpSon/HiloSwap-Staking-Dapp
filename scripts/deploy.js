async function main() {
  ...
  const staking = await Staking.deploy(121976, {
  ...
  await staking
    .connect(owner)
    .addToken("WrappedBitcoin", "WBTC", wrappedBitcoin.address, 1681083, 500);
  await staking
    .connect(owner)
    .addToken("WrappedEther", "WETH", wrappedEther.address, 121976, 1000);
  ...
  await staking
    .connect(owner)
    .stakeToken("LINK", ethers.utils.parseEther("100"));
  ...
  await staking
    .connect(owner)
    .stakeToken("WBTC", ethers.utils.parseEther("2"));
  ...
  await staking
    .connect(owner)
    .stakeToken("WBTC", ethers.utils.parseEther("10"));
  ...
  await staking
    .connect(owner)
    .stakeToken("WETH", ethers.utils.parseEther("10"));
